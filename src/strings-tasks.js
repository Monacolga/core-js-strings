...
function findLongestWord(sentence) {
  ...
  let currentElement = sentenceArray[0];
  for (let i = 0; i < sentenceArray.length; i += 1) {
    if (currentElement.length < sentenceArray[i].length) {
      currentElement = sentenceArray[i];
    }
  }
...
function reverseWords(str) {
  ...
  const reversedArray = arrayStr.map((elem) =>
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toUpperCase()) {
      invertedArray.push(str[i].toLowerCase());
    } else if (str[i] === str[i].toLowerCase()) {
      invertedArray.push(str[i].toUpperCase());
    }
  }
